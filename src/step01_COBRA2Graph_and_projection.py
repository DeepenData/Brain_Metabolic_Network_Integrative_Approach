...
INPUTS: 
      INPUT_MODEL : a .json cobra model
OUTPUT: 
      ./tmp/graph.graphml : a graph, in .graphml format for NetworkX and Graph-tool compatibility
      # TODO #2 : parametrize the .graphml output, same as the model name
"""

# %% --- IMPORTA EL MODELO

INPUT_MODEL = "./data/models/stimulated_NAMU.json"  # TODO: hacer esto una variable ambiental
...
# TODO: #1 Change import info print for more info

projected_S_matrix = np.matmul(
    S_matrix.T, S_matrix
)  # Multiplicacion por la derecha para proyectar en el espacio de las reacciones
print(
    "Dimensiones de la matriz:",
    projected_S_matrix.shape,  # Dimensiones, deberian ser cuadradas
    "Valores no-cero:",
    np.count_nonzero(projected_S_matrix),
)  # Valores no-cero, deberian mantenerse
# %%
print("Removiendo la diagonal...")
np.fill_diagonal(projected_S_matrix, 0)  # Llena las diagonales de ceros.

projected_S_matrix = (projected_S_matrix != 0).astype(
    int)  # No require importar SKLearn
...
print(
    ...
    projected_S_matrix.shape,  # Dimensiones, deberian ser cuadradas
    "Valores no-cero:",
    np.count_nonzero(projected_S_matrix),  # Valores no-cero, deberian mantenerse
    "Valores unicos:",
    np.unique(projected_S_matrix),
)  # Deberian ser [0 1]

G = nx.convert_matrix.from_numpy_matrix(
    projected_S_matrix
)  # Convierte la matriz a numpy
...
nx.write_graphml(G, "./tmp/graph.graphml", named_key_ids=True) # INFO: El primer paso es crear un 
