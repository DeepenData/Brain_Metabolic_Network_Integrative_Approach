...
get_optimality_values <- function(data){
  ...
  data %>% select(c("ID", "Sensitivity", 'Flux'))   %>% column_to_rownames('ID') %>% #abs %>% as.matrix() %>% pseudoLog10  -> data.log
    ...
    mutate(Flux = std_rescale(Flux), Sensitivity = std_rescale(Sensitivity)) %>% 
    mutate(total = sqrt(abs(Flux)^2 + abs(Sensitivity)^2)) %>% filter(total > 1e-14)   -> data.log
  
  
  data.log  %>% set_rownames(rownames(data.log))-> data.log.rescaled
  merge(data.log.rescaled %>% rownames_to_column('ID') ,  data[, c('ID','Node type')] ) -> data.log.rescaled
  
  
  data.log.rescaled -> data.log.rescaled_total
